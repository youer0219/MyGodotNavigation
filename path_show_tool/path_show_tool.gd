...
func draw_path(node:Node,paths:Array[Vector2],path_color:Color = Color.ALICE_BLUE ,path_circle_radius:float = 1.0):
	...
	var path_show_executor := find_path_show_executor_by_id(node_id)
	if path_show_executor == null:
		path_show_executor = find_spare_path_show_executor()
		path_show_executor.path_requester_id = EncodedObjectAsID.new()
		path_show_executor.path_requester_id.object_id = node_id
		node.tree_exiting.connect(clear_draw_path.bind(node))
...
func clear_draw_path(node:Node):
	var node_id := node.get_instance_id()
	var path_show_executor := find_path_show_executor_by_id(node_id)
...
func find_path_show_executor_by_id(id:int)->PathShowExecutor:
	...
	for path_show_executor in get_path_show_executors():
...
func find_spare_path_show_executor()->PathShowExecutor:
